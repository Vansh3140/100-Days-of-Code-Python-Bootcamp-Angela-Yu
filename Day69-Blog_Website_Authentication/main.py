...
from sqlalchemy.orm import relationship, DeclarativeBase, Mapped, mapped_column, query
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if check_user:
            flash('You are already registered kindly login')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        curr_user = db.session.execute(db.select(User).where(User.email == email)).scalar()
        if curr_user:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash('Login/Register Before Commenting', 'danger')
            return redirect(url_for('login'))
        new_comment = Comment(
            text=form.body.data,
        ...
        db.session.commit()
    all_comments = db.session.query(Comment)
    return render_template("post.html", post=requested_post, form=form, all_comments=all_comments, logged_in=current_user.is_authenticated)
